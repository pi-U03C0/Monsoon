...
{
  LOG("Poping Value %020llu from %d",MONSOON_LOG_LOW_DEBUG,255,Queue -> Items[Queue -> PopPos],Queue -> PopPos);
  if (!Queue)
  {
    LOG("Queue was NULL",MONSOON_LOG_ERROR,MONSOON_LOG_WAS_NULL);
    return False;
...
{
  LOG("Pushing %020llu To Queue at pos %d",MONSOON_LOG_LOW_DEBUG,255,Item,Queue -> PushPos);
  if (!Queue)
  {
    LOG("Queue was NULL",MONSOON_LOG_ERROR,MONSOON_LOG_WAS_NULL);
    return False;
