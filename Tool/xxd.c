...
//    -v          show version: "xxd 2022-01-14 by Juergen Weigert et al. (Win32)".
...
static int fgetc_or_die(FILE* fpi)
{
  int c = fgetc(fpi);
...
static void fputc_or_die(int c, FILE* fpo)
{
  if(fputc(c, fpo) == EOF)
  {
    perror_exit(3);
 ...
 */
static void xxdline(FILE* fp, const char* l, const int nz)
{
